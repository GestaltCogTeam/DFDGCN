    ...
    with open(url, "r") as f:
        ...
        for row in reader:
            if len(row) != 3:
                continue
            print(row)
            index, lon, lat = int(row[0]), float(row[1]), float(row[2])
            lst.append([lon,lat])
    return np.array(lst)
...
def generate_data(args: argparse.Namespace):
    ...
    print(len(lon_lat), 'location 1111111')
...
if __name__ == "__main__":
    ...
    LOCATION_FILE_PATH = "datasets/raw_data/{0}/graph_sensor_locations_bay.csv".format(DATASET_NAME)

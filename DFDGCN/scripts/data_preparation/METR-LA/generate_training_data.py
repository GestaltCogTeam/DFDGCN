    ...
    with open(url, "r") as f:
        ...
        for row in reader:
            ...
            if "i" in str(row[0]):
                ...
                index, id, lon, lat = int(row[0]), int(row[1]), float(row[2]), float(row[3])
                lst.append([lon,lat])
    return np.array(lst)
...
if __name__ == "__main__":
    ...
    LOCATION_FILE_PATH = "D:/myfile/BasicTS/datasets/raw_data/{0}/graph_sensor_locations.csv".format(DATASET_NAME)
